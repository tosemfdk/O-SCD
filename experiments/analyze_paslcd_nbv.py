...
def main(path="experiments/paslcd_nbv_results.csv"):
    ...
    configs = [("uniform", 3), ("random", 3), ("nbv", 3),
               ("uniform", 5), ("random", 5), ("nbv", 5)]
    ...
    # head-to-head nbv vs uniform per budget
    for K in (3, 5):
        diffs = []
        for s in scenes:
            n, u = get(s, "nbv", K), get(s, "uniform", K)
            if n and u:
        ...
        print(f"\nnbv vs uniform @K={K}: mean {d.mean():+.4f} mIoU, "
