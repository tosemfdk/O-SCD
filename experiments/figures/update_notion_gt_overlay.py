# Swap the Notion page's 10 montages for the 4-row versions (each frame row
# followed by its GT change-mask row with the changed-pixel share) and keep
# the explanation in sync.
import json
import os
import subprocess
...
NOTE = " 각 프레임 아래 행 = 그 프레임의 GT 변화 마스크 (% = 변화 픽셀 비율)."
CAP = {
...
api("PATCH", f"https://api.notion.com/v1/blocks/{mexp}", {"paragraph": {"rich_text": [
    ...
    rt("최강", bold=True),
    rt(" uniform offset — 체리피킹 없는 비교. 각 프레임 아래 행이 그 프레임의 GT 변화 마스크다). "
       "Cantina의 oracle 5장은 전부 후반부(12–21)로, 변화가 몰린 카운터 구역을 서로 다른 각도에서 "
       "재관측한다 — 최강 uniform과 겹치는 프레임이 단 한 장도 없이 +25%. Playground는 변화가 "
       "화면의 0–1%뿐인 씬인데, oracle은 그 작은 변화가 실제로 잡히는 근거리 프레임에 예산을 "
       "몰아준다(+22%). 마스크 행이 드러내는 중요한 사실 하나: "),
