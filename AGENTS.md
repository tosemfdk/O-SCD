...
- 이 checkpoint에는 MCMC, SGLD, relocation, unrestricted Gaussian birth, generic densification/pruning은 포함하지 않는다. Production temporal model의 기본 topology는 fixed이며, dynamic topology는 위 active residual density ablation runner에서만 명시적으로 사용한다.
